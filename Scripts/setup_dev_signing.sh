...
# Check if we already have a Clawbar development certificate
CERT_NAME="Clawbar Development"
...
openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 \
    -nodes -keyout /tmp/clawbar-dev.key -out /tmp/clawbar-dev.crt \
    -config "$TEMP_CONFIG" 2>/dev/null

# Convert to PKCS12 format
openssl pkcs12 -export -out /tmp/clawbar-dev.p12 \
    -inkey /tmp/clawbar-dev.key -in /tmp/clawbar-dev.crt \
    -passout pass: 2>/dev/null

# Import into keychain
security import /tmp/clawbar-dev.p12 -k ~/Library/Keychains/login.keychain-db -T /usr/bin/codesign -T /usr/bin/security

# Clean up temporary files
rm -f /tmp/clawbar-dev.{key,crt,p12}
