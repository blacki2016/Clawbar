...
echo "$APP_STORE_CONNECT_API_KEY_P8" | sed 's/\\n/\n/g' > /tmp/clawbar-api-key.p8
trap 'rm -f /tmp/clawbar-api-key.p8 /tmp/${APP_NAME}Notarize.zip' EXIT
...
APP_ENTITLEMENTS="${ENTITLEMENTS_DIR}/Clawbar.entitlements"
WIDGET_ENTITLEMENTS="${ENTITLEMENTS_DIR}/ClawbarWidget.entitlements"

echo "Signing with $APP_IDENTITY"
if [[ -f "$APP_BUNDLE/Contents/Helpers/clawbar" ]]; then
...
if [[ -d "$APP_BUNDLE/Contents/PlugIns/ClawbarWidget.appex" ]]; then
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    ...
    "$APP_BUNDLE/Contents/PlugIns/ClawbarWidget.appex/Contents/MacOS/ClawbarWidget"
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    --entitlements "$WIDGET_ENTITLEMENTS" \
    "$APP_BUNDLE/Contents/PlugIns/ClawbarWidget.appex"
fi
codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
