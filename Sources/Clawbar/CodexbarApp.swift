...
struct ClawbarApp: App {
    ...
    init() {
        ...
        let storedLevel = ClawbarLog.parseLevel(UserDefaults.standard.string(forKey: "debugLogLevel")) ?? .verbose
        let level = ClawbarLog.parseLevel(env["CODEXBAR_LOG_LEVEL"]) ?? storedLevel
        ClawbarLog.bootstrapIfNeeded(.init(
            destination: .oslog(subsystem: "com.steipete.clawbar"),
            level: level,
            json: false))
        ...
        ClawbarLog.logger(LogCategories.app).info(
            "Clawbar starting",
            metadata: [
                "version": version,
                "build": build,
        ...
        ClawbarLog.setLogLevel(settings.debugLogLevel)
    ...
    var body: some Scene {
        ...
        WindowGroup("ClawbarLifecycleKeepalive") {
            HiddenWindowView()
        }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func ensureStatusController() {
        ...
        ClawbarLog.logger(LogCategories.app)
