...
struct ClawbarConfigMigrator {
    ...
    static func loadOrMigrate(
        configStore: ClawbarConfigStore,
        userDefaults: UserDefaults,
        stores: LegacyStores) -> ClawbarConfig
    {
        let log = ClawbarLog.logger(LogCategories.configMigration)
        let existing = try? configStore.load()
        var config = (existing ?? ClawbarConfig.makeDefault()).normalized()
    ...
    private static func applyProviderOrder(_ raw: [String], config: ClawbarConfig) -> ClawbarConfig {
    ...
    private static func applyProviderToggles(
        ...
        config: ClawbarConfig) -> ClawbarConfig
    {
        var updated = config
