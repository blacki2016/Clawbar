...
extension SettingsStore {
    private func updateConfig(reason: String, mutate: (inout ClawbarConfig) -> Void) {
    ...
    func updateProviderTokenAccounts(_ accounts: [UsageProvider: ProviderTokenAccountData]) {
        let summary = accounts
            ...
            .joined(separator: ",")
        ClawbarLog.logger(LogCategories.tokenAccounts).info(
    ...
    func reloadConfig(reason: String) {
        ...
        do {
            ...
            ClawbarLog.logger(LogCategories.configStore).error("Failed to reload config: \(error)")
        }
    }

    func applyExternalConfig(_ config: ClawbarConfig, reason: String) {
        guard !self.configLoading else { return }
    ...
    private func bumpConfigRevision(_ context: ConfigChangeContext) {
        ...
        ClawbarLog.logger(LogCategories.settings)
    ...
    func schedulePersistConfig() {
        ...
        if Self.isRunningTests {
            do {
                ...
                ClawbarLog.logger(LogCategories.configStore).error("Failed to persist config: \(error)")
            }
            return
        }
