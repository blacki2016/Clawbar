...
final class SettingsStore {
    static let sharedDefaults = UserDefaults(suiteName: "group.com.steipete.clawbar")
    ...
    @ObservationIgnored let configStore: ClawbarConfigStore
    @ObservationIgnored var config: ClawbarConfig
    ...
    init(
        ...
        configStore: ClawbarConfigStore = ClawbarConfigStore(),
    ...
    {
        let legacyStores = ClawbarConfigMigrator.LegacyStores(
        ...
        let config = ClawbarConfigMigrator.loadOrMigrate(
        ...
        ClawbarLog.setFileLoggingEnabled(self.debugFileLoggingEnabled)
...
extension SettingsStore {
    private static func loadDefaultsState(userDefaults: UserDefaults) -> SettingsDefaultsState {
        ...
        let debugLogLevelRaw = userDefaults.string(forKey: "debugLogLevel") ?? ClawbarLog.Level.verbose.rawValue
...
extension SettingsStore {
    var configSnapshot: ClawbarConfig {
        _ = self.configRevision
        return self.config
    }

    func updateProviderState(config: ClawbarConfig) {
    ...
    func setProviderEnabled(provider: UsageProvider, metadata _: ProviderMetadata, enabled: Bool) {
        ClawbarLog.logger(LogCategories.settings).debug(
