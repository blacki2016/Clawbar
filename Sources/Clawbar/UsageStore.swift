...
final class UsageStore {
    ...
    @ObservationIgnored private let sessionQuotaLogger = ClawbarLog.logger(LogCategories.sessionQuota)
    @ObservationIgnored let openAIWebLogger = ClawbarLog.logger(LogCategories.openAIWeb)
    @ObservationIgnored private let tokenCostLogger = ClawbarLog.logger(LogCategories.tokenCost)
    @ObservationIgnored let augmentLogger = ClawbarLog.logger(LogCategories.augment)
    @ObservationIgnored let providerLogger = ClawbarLog.logger(LogCategories.providers)
...
extension UsageStore {
    func debugDumpClaude() async {
        ...
        let url = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("clawbar-claude-probe.txt")
    ...
    func dumpLog(toFileFor provider: UsageProvider) async -> URL? {
        ...
        let filename = "clawbar-\(provider.rawValue)-probe.txt"
        ...
        fallbackCookieHeader: String) async -> ClaudeDebugLogConfiguration
    {
        await MainActor.run {
            ...
            return ClaudeDebugLogConfiguration(
                runtime: ClawbarCore.ProviderRuntime.app,
