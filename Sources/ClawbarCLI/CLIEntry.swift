...
enum ClawbarCLI {
    ...
    private static func bootstrapLogging(values: ParsedValues) {
        ...
        ClawbarLog.bootstrapIfNeeded(.init(destination: .stderr, level: level, json: isJSON))
    }

    static func resolvedLogLevel(verbose: Bool, rawLevel: String?) -> ClawbarLog.Level {
        ClawbarLog.parseLevel(rawLevel) ?? (verbose ? .debug : .error)
    }
