...
extension ClawbarCLI {
    static func decodeProvider(from values: ParsedValues, config: ClawbarConfig) -> ProviderSelection {
    ...
    static func loadConfig(output: CLIOutputPreferences) -> ClawbarConfig {
        let store = ClawbarConfigStore()
        do {
            if let existing = try store.load() {
                return existing
            }
            return ClawbarConfig.makeDefault()
        } catch {
