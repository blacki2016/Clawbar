...
public enum ClawbarLog {
    ...
    public static func logger(_ category: String) -> ClawbarLogger {
        let logger = Logger(label: "com.steipete.clawbar.\(category)")
        return ClawbarLogger { level, message, metadata in
...
public struct ClawbarLogger: Sendable {
    private let logFn: @Sendable (ClawbarLog.Level, String, [String: String]?) -> Void

    fileprivate init(_ logFn: @escaping @Sendable (ClawbarLog.Level, String, [String: String]?) -> Void) {
        self.logFn = logFn
    }
