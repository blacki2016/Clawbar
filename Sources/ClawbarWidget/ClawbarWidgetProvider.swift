...
struct ClawbarWidgetEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
    let snapshot: WidgetSnapshot
}

struct ClawbarCompactEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct ClawbarSwitcherEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct ClawbarTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> ClawbarWidgetEntry {
        ClawbarWidgetEntry(
            date: Date(),
            provider: .codex,
    ...
    func snapshot(for configuration: ProviderSelectionIntent, in context: Context) async -> ClawbarWidgetEntry {
        let provider = configuration.provider.provider
        return ClawbarWidgetEntry(
            date: Date(),
            provider: provider,
    ...
    func timeline(
        ...
        in context: Context) async -> Timeline<ClawbarWidgetEntry>
    {
        let provider = configuration.provider.provider
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.snapshot()
        let entry = ClawbarWidgetEntry(date: Date(), provider: provider, snapshot: snapshot)
        let refresh = Date().addingTimeInterval(30 * 60)
        return Timeline(entries: [entry], policy: .after(refresh))
    }
}

struct ClawbarSwitcherTimelineProvider: TimelineProvider {
    func placeholder(in context: Context) -> ClawbarSwitcherEntry {
        let snapshot = WidgetPreviewData.snapshot()
        let providers = self.availableProviders(from: snapshot)
        return ClawbarSwitcherEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (ClawbarSwitcherEntry) -> Void) {
        completion(self.makeEntry())
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<ClawbarSwitcherEntry>) -> Void) {
        let entry = self.makeEntry()
        let refresh = Date().addingTimeInterval(30 * 60)
        completion(Timeline(entries: [entry], policy: .after(refresh)))
    }

    private func makeEntry() -> ClawbarSwitcherEntry {
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.snapshot()
        let providers = self.availableProviders(from: snapshot)
...
struct ClawbarCompactTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> ClawbarCompactEntry {
        ClawbarCompactEntry(
            date: Date(),
            provider: .codex,
            metric: .credits,
    ...
    func snapshot(for configuration: CompactMetricSelectionIntent, in context: Context) async -> ClawbarCompactEntry {
        let provider = configuration.provider.provider
        return ClawbarCompactEntry(
            date: Date(),
            provider: provider,
    ...
    func timeline(
        ...
        in context: Context) async -> Timeline<ClawbarCompactEntry>
    {
        let provider = configuration.provider.provider
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.snapshot()
        let entry = ClawbarCompactEntry(
            date: Date(),
            provider: provider,
            metric: configuration.metric,
            snapshot: snapshot)
