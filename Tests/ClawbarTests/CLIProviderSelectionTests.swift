...
struct CLIProviderSelectionTests {
    ...
    func `help includes gemini and all`() {
        let usage = ClawbarCLI.usageHelp(version: "0.0.0")
        let root = ClawbarCLI.rootHelp(version: "0.0.0")
    ...
    func `provider selection respects override`() {
        let selection = ClawbarCLI.providerSelection(rawOverride: "gemini", enabled: [.codex, .claude])
        #expect(selection.asList == [.gemini])
    }

    @Test
    func `provider selection uses all when enabled`() {
        let selection = ClawbarCLI.providerSelection(
            rawOverride: nil,
    ...
    func `provider selection uses custom for codex and gemini`() {
        ...
        let selection = ClawbarCLI.providerSelection(rawOverride: nil, enabled: enabled)
        #expect(selection.asList == enabled)
    }

    @Test
    func `provider selection accepts kiro alias`() {
        let selection = ClawbarCLI.providerSelection(rawOverride: "kiro-cli", enabled: [.codex])
