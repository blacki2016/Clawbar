...
1) **API token** (preferred)
   - Set in Preferences → Providers → MiniMax (stored in `~/.clawbar/config.json (legacy fallback: ~/.codexbar/config.json)`) or `MINIMAX_API_KEY`.
...
5) **Manual session cookie header** (optional override)
   - Stored in `~/.clawbar/config.json (legacy fallback: ~/.codexbar/config.json)` via Preferences → Providers → MiniMax (Cookie source → Manual).
